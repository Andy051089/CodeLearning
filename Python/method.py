...
print(f' train_normal_tree Accuracy Score: {normal_tree.score(xtrain, ytrain)}')
print(f' test_normal_tree Accuracy Score: {normal_tree.score(xtest, ytest)}')
print(f' train_normal_tree F1 score: {metrics.f1_score(ytrain, ytrain_normal_tree_pred)}')
print(f' test_normal_tree F1 score: {metrics.f1_score(ytest, ytest_normal_tree_pred)}')
print(f' train_normal_tree AUC score: {metrics.roc_auc_score(ytrain, ytrain_normal_tree_pred_proba)}')
print(f' test_normal_tree AUC score: {metrics.roc_auc_score(ytest, ytest_normal_tree_pred_proba)}')
# 創建模型
...
bayes_tuning_tree_search =  BayesSearchCV(
    ...
    scoring = scoring)
# 把資料FIT進去找最佳超參數
bayes_tuning_tree_search.fit(xtrain, ytrain)
# 最佳超參數
...
#     class_weight = class_weight)
# 把資料FIT進最佳超參數的決策樹
best_tuning_tree_model = bayes_tuning_tree.fit(xtrain, ytrain)
#把訓練測試資料FIT模型做預測看結果
...
print(f' train_tuning_tree Accuracy Score: {best_tuning_tree_model.score(xtrain, ytrain)}')
print(f' test_tuning_tree Accuracy Score: {best_tuning_tree_model.score(xtest, ytest)}')
print(f' train_tuning_tree F1 score: {metrics.f1_score(ytrain, ytrain_best_tuning_tree_pred)}')
print(f' test_tuning_tree F1 score: {metrics.f1_score(ytest, ytest_best_tuning_tree_pred)}')
print(f' train_tuning_tree AUC score: {metrics.roc_auc_score(ytrain, ytrain_best_tuning_tree_pred_proba)}')
print(f' test_tuning_tree AUC score: {metrics.roc_auc_score(ytest, ytest_best_tuning_tree_pred_proba)}')
#%%隨機森林
'''
...
normal_forest = RandomForestClassifier(
    ...
    min_samples_split = 10,
    ccp_alpha = 0.05)
normal_forest.fit(xtrain, ytrain)
...
print(f' train_normal_forest Accuracy Score: {normal_forest.score(xtrain, ytrain)}')
print(f' test_normal_forest Accuracy Score: {normal_forest.score(xtest, ytest)}')
print(f' train_normal_forest F1 score: {metrics.f1_score(ytrain, ytrain_normal_forest_pred)}')
print(f' test_normal_forest F1 score: {metrics.f1_score(ytest, ytest_normal_forest_pred)}')
print(f' train_normal_forest AUC score: {metrics.roc_auc_score(ytrain, ytrain_normal_forest_pred_proba)}')
print(f' test_normal_forest AUC score: {metrics.roc_auc_score(ytest, ytest_normal_forest_pred_proba)}')
# 建模型
...
bayes_tuning_forest_search =  BayesSearchCV(
    ...
    scoring = scoring)    
#把資料FIT進去找最佳超參數
bayes_tuning_forest_search.fit(xtrain, ytrain)
#最佳參數
...
# bayes_tuning_forest = RandomForestClassifier(
    ...
    # n_estimators = 156)
# 把資料FIT進最佳超參數的隨機森林
best_tuning_forest_model = bayes_tuning_forest.fit(xtrain, ytrain)
# 把訓練測試資料FIT模型做預測看結果
...
print(f' train_forest Accuracy Score: {best_tuning_forest_model.score(xtrain, ytrain)}')
print(f' test_forest Accuracy Score: {best_tuning_forest_model.score(xtest, ytest)}')
print(f' train_forest F1 score: {metrics.f1_score(ytrain, ytrain_best_tuning_forest_pred)}')
print(f' test_forest F1 score: {metrics.f1_score(ytest, ytest_best_tuning_forest_pred)}')
print(f' train_forest AUC score: {metrics.roc_auc_score(ytrain, ytrain_best_tuning_forest_pred_proba)}')
print(f' test_forest AUC score: {metrics.roc_auc_score(ytest, ytest_best_tuning_forest_pred_proba)}')
#%%XGBoost
'''
...
print(f' train_normal_xgb Accuracy Score: {normal_xgb.score(xtrain, ytrain)}')
print(f' test_normal_xgb Accuracy Score: {normal_xgb.score(xtest, ytest)}')
print(f' train_normal_xgb F1 score: {metrics.f1_score(ytrain, ytrain_normal_xgb_pred)}')
print(f' test_normal_xgb F1 score: {metrics.f1_score(ytest, ytest_normal_xgb_pred)}')
print(f' train_normal_xgb AUC score: {metrics.roc_auc_score(ytrain, ytrain_normal_xgb_pred_proba)}')
print(f' test_normal_xgb AUC score: {metrics.roc_auc_score(ytest, ytest_normal_xgb_pred_proba)}')
# 創建模型
...
# 把資料FIT進最佳超參數的xgb
best_tuning_xgb_model = bayes_tuning_xgb.fit(X_train, y_train)
# 把訓練測試資料FIT模型做預測看結果
ytrain_best_tuning_xgb_pred = best_tuning_xgb_model.predict(X_train)
ytest_best_tuning_xgb_pred = best_tuning_xgb_model.predict(xtest)
# 把訓練測試資料FIT模型做預測看結果的機率
ytrain_best_tuning_xgb_pred_proba = best_tuning_xgb_model.predict_proba(X_train)[:,1]
ytest_best_tuning_xgb_pred_proba = best_tuning_xgb_model.predict_proba(xtest)[:,1]
print(f' train_tuning_xgb Accuracy Score: {best_tuning_xgb_model.score(X_train, y_train)}')
print(f' test_tuning_xgb Accuracy Score: {best_tuning_xgb_model.score(xtest, ytest)}')
print(f' train_tuning_xgb F1 score: {metrics.f1_score(y_train, ytrain_best_tuning_xgb_pred)}')
print(f' test_tuning_xgb F1 score: {metrics.f1_score(ytest, ytest_best_tuning_xgb_pred)}')
print(f' train_tuning_xgb AUC score: {metrics.roc_auc_score(y_train, ytrain_best_tuning_xgb_pred_proba)}')
print(f' test_tuning_xgb AUC score: {metrics.roc_auc_score(ytest, ytest_best_tuning_xgb_pred_proba)}')
'''
1.一定要切割在切割，沒有驗證集不能做
...
normal_ann = keras.Sequential([
    keras.layers.Dense(64, input_shape = (xtrain.shape[1],), activation = 'relu'),
...
ytrain_best_normal_ann_pred = normal_ann.predict(xtrain)
ytest_best_normal_ann_pred = normal_ann.predict(xtest)
# >0.5 : 1, <0.5 : 0
ytrain_normal_ann_predicted_labels = (ytrain_best_normal_ann_pred > threshold).astype(int)
ytest_normal_ann_predicted_labels = (ytest_best_normal_ann_pred > threshold).astype(int)
print(f' train_normal_ann Accuracy Score: {metrics.accuracy_score(ytrain, ytrain_normal_ann_predicted_labels)}')
print(f' test_normal_ann Accuracy Score: {metrics.accuracy_score(ytest, ytest_normal_ann_predicted_labels)}')
print(f' train_normal_ann F1 score: {metrics.f1_score(ytrain, ytrain_normal_ann_predicted_labels)}')
print(f' test_normal_ann F1 score: {metrics.f1_score(ytest, ytest_normal_ann_predicted_labels)}')
print(f' train_normal_ann AUC score: {metrics.roc_auc_score(ytrain, ytrain_best_normal_ann_pred)}')
print(f' test_normal_ann AUC score: {metrics.roc_auc_score(ytest, ytest_best_normal_ann_pred)}')
...
bayes_tuning_ann = tuning_ann.BayesianOptimization(
    tuning_ann,
    objective = Objective(
...
ytrain_best_tuning_ann_pred = best_bayes_tuning_ann_model.predict(xtrain)
ytest_best_tuning_ann_pred = best_bayes_tuning_ann_model.predict(xtest)
# >0.5 : 1, <0.5 : 0
ytrain_tuning_ann_predicted_labels = (ytrain_best_tuning_ann_pred > threshold).astype(int)
ytest_tuning_ann_predicted_labels = (ytest_best_tuning_ann_pred > threshold).astype(int)
print(f' train_tuning_ann Accuracy Score: {metrics.accuracy_score(ytrain, ytrain_tuning_ann_predicted_labels)}')
print(f' test_tuning_ann Accuracy Score: {metrics.accuracy_score(ytest, ytest_tuning_ann_predicted_labels)}')
print(f' train_tuning_ann F1 score: {metrics.f1_score(ytrain, ytrain_tuning_ann_predicted_labels)}')
print(f' test_tuning_ann F1 score: {metrics.f1_score(ytest, ytest_tuning_ann_predicted_labels)}')
print(f' train_tuning_ann AUC score: {metrics.roc_auc_score(ytrain, ytrain_best_tuning_ann_pred)}')
print(f' test_tuning_ann AUC score: {metrics.roc_auc_score(ytest, ytest_best_tuning_ann_pred)}')
#%%CNN
'''
...
Convolution Layer，把各項特徵結合在一起，組合成各項特徵的結合，最終把各項特徵壓扁
結合，作為ANN的訓練資料。在特徵偵測擷取時，以幾*幾的方格作為每個特徵的大小，以
strides設定的移動格數把圖的每個地方都至少輪過一次，但較靠近圖中央會被方格反覆偵測
到，而周圍次數少，padding是在原圖的周圍加上空白的格子，讓周圍的圖可以增加偵測到的
次數。做不做Pooling在後面特徵擷取計算時都會得到相同大小。為了增加圖像的預測能力，
訓練資料的圖片是可以平移、旋轉、縮放、亮度調整等。

2.一次epochs，會從train_generator拿設定的batch_size張圖，進行訓練後再繼續拿設定的
batch_size張圖，直到張數達原本資料及中圖片的數目。所拿的圖不一定是原本資料夾的圖。
訓練完後進入驗證過程，從test_generator拿設定的batch_size張圖，直到資料夾中的張數
，再進入到下一次epochs。
...
for img_path, class_label in train_data:
    dest_path = os.path.join(class_temp_dirs[class_label], os.path.basename(img_path))
...
# 每個類別生成幾張
    pipeline.sample(300)
# 獲取增強後的圖片路徑
    augmented_dir = os.path.join(class_temp_dir, "output")
    for img_name in os.listdir(augmented_dir):
        img_path = os.path.join(augmented_dir, img_name)
        augmented_train_data.append((img_path, class_label))
# 合併原始訓練數據和增強後的數據
...
cnnmodel = keras.Sequential([
    layers.Conv2D(  
        ...
        input_shape = train_generator.image_shape, strides=(1, 1),
        padding='same'),
...
cnnmodel.fit(train_generator, epochs = 50, validation_data=val_generator)
...
ytrain_cnn_pred = cnnmodel.predict(cnn_train_images)
ytest_cnn_pred = cnnmodel.predict(cnn_test_images)
# >0.5 : 1, <0.5 : 0
...
print(f' train_cnn Accuracy Score: {metrics.accuracy_score(cnn_train_labels, ytrain_cnn_predicted_labels)}')
print(f' test_cnn Accuracy Score: {metrics.accuracy_score(cnn_test_labels , ytest_cnn_predicted_labels)}')
print(f' train_cnn F1 score: {metrics.f1_score(cnn_train_labels, ytrain_cnn_predicted_labels)}')
print(f' test_cnn F1 score: {metrics.f1_score(cnn_test_labels ,ytest_cnn_predicted_labels)}')
print(f' train_cnn AUC score: {metrics.roc_auc_score(cnn_train_labels, ytrain_cnn_pred)}')
print(f' test_cnn AUC score: {metrics.roc_auc_score(cnn_test_labels , ytest_cnn_pred)}')

# 建模型
def cnn_model(hp):
    model = keras.Sequential()
    nums_set_layers = hp.Int('nums_set_layers', 1, 4, 1)
    for i in range(nums_set_layers):
        if i == 0:
            model.add(layers.Conv2D(
                filters = hp.Int('start_filter', 16, 64, 8),
...
# 從訓練及測試生成器產生圖並把圖及標籤加入list後轉成array
cnn_train_images, cnn_train_labels = [], []
for i in range(len(train_generator)):
    x_batch, y_batch = train_generator[i]
    cnn_train_images.extend(x_batch)
